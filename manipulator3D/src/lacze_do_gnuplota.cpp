...
namespace PzG
	...
	while (((IloscPrzeslanych = write(_Wejscie_GNUPlota, Komunikat, Ilosc))
		!= -1) && Ilosc > 0)
	    {
	...
	if (Ilosc > 0)
	    {
	    KomunikatBledu(
		    "!!! Przeslanie polecenia do GNUPlota nie powiodlo sie.");
	    return false;
	    }
	return true;
	}
	...
	_InfoPlikow.push_back(
		InfoPlikuDoRysowania(NazwaPliku, RodzRys, Szerokosc));
	return rezultat;
	}
...
#define ROZ_LINII   120
	...
	case 0:
		...
		{
		KomunikatBledu(
			"### Blad przy zamykaniu strumienia wejsciowego.");
		exit(1);
		}
		...
		{
		KomunikatBledu(
			"### Blad zamkniecia duplikatu kanalu wejsciowego.");
		exit(1);
		}
	    if (close(indesk[STDOUT]) == -1)
		{
		KomunikatBledu(
			"### Blad zamkniecia duplikatu kanalu wyjsciowego.");
		exit(1);
		}
	    char Tab[3];
	    read(STDIN, Tab, 1);
	    ...
	    KomunikatBledu(
		    "!!! Nastapilo przerwanie dzialania procesu potomnego.");
	...
	default:
		...
		{
		KomunikatBledu(
			" Blad zamkniecia outpipe[STDIN], inpipe[STDOUT].");
		}
    ...
    /*!
     * Tworzy napis b�d�cy parametrami dla polecenie \e plot programu,
     * \e gnuplot. Parametry te pozwalaj� na rysowanie bry�, kt�rych
     *  wsp�rz�dne wierzcho�k�w zawarte s� w plikach.
     *  Nazwy tych plik�w musz� by� wcze�niej do��czone do kolejki
     *  plik�w poprzez zastosowanie polecenia
     *   \link LaczeDoGNUPlota::DodajNazwe DodajNazwe\endlink.
     *
     * \param Polecenie - dopisywana jest do niego sekwencja znak�w
     *                  tworz�cych parametry dla polecenia \e plot.
     * \param Sep - zawiera znak separatora mi�dzy poszczeg�lnymi
     *              parametrami. Je�eli parametry listy nazw plik�w
     *              s� generowane jako pierwsze, to zmienna ta musi
     *              by� wska�nikiem do wska�nika na �a�cuch: " ".
     * \retval true - je�li lista nazw plik�w nie jest pusta.
     * \retval false - w przypadku przeciwnym.
     * \post Je�eli  lista nazw plik�w nie jest pusta, to poprzez
     *              parametr \e Sep zostaje udost�pniony �a�cuch: ", ".
     */
    bool LaczeDoGNUPlota::DopiszRysowanieZPlikow(std::string &Polecenie,
	    char const **Sep)
	{
	if (_InfoPlikow.empty())
	    return false;
    ...
    //                 Nie oznacza to jednak, ze proces rysowania zako�czy�
    //                 si� pomy�lnie.
	 ...
	 */
	TrzebaRysowac = DopiszRysowanieZPlikow(Polecenie, &Sep)
		|| TrzebaRysowac;
